package baseball.constant;

public class SystemMessage {
    public final static String NUMBER_LENGTH_NOT_CORRECT = "입력한 숫자의 길이가 올바르지 않습니다.";
    public final static String HAS_SAME_NUMBER = "중복된 숫자가 포함되어있습니다.";
    public final static String STRING_IS_NOT_NUMBER = "숫자가 아닌 문자가 포함되어 있습니다.";

    public final static String SET_USER_NUMBER_MESSAGE = "숫자를 입력해주세요 : ";

    public final static String BALL_MESSAGE = "볼";
    public final static String STRIKE_MESSAGE = "스트라이크";
    public final static String NOTHING_MESSAGE = "낫싱";
}
