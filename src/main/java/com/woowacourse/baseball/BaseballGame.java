...
public class BaseballGame {
    ...
    static int inputPlayerNumber(Scanner scanner) {
        ...
        /* 입력받은 수가 1부터 9까지의 3자리 수가 아니면 다시 입력하도록 하고, 그렇지 않으면 사용자가 입력한 수를 반환한다. */
        if (s.matches("[1-9]{3}")) {
    ...
    static int restartOrFinishGame(Scanner scanner) {
        ...
        do {
            ...
            /* 입력받은 수가 1 또는 2가 아니면 다시 입력하도록 한다. */
            if (!s.matches("[1,2]")) {
                continue;
            }
    ...
    static void playGame(Scanner scanner) {
        ...
        do {
            compareResult = compareNumber(computerNumber, inputPlayerNumber(scanner));
