...
- 메소드
  ...
  - `checkAnswer`: 유저가 제시한 답이 정답인지 확인한다.(`findIndexOfList` 호출) 스트라이크와 볼의 개수를 `int[]`로 반환한다.
...
- `testCheckAnswer`: 정답이 `482`인 GameManager를 생성시킨 후, 여러 입력을 받고 스트라이크와 볼의 수가 제대로 나오는지 확인한다.
