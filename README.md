...
## 나의 기능구현
- 사용자
  - 사용자로부터 세 자리의 수를 입력받는다.
  - 사용자가 잘못된 값을 입력할 경우 예외를 발생시킨 후 종료시킨다.
- 컴퓨터
  - 1부터 9까지 서로 다른 수로 이루어진 3자리의 수를 랜덤으로 만든다.
- 진행
  - 사용자가 입력한 수와 컴퓨터가 생성한 수를 비교한다.
  - 비교 후, 볼/스트라이크/낫싱 여부 파악 후 사용자에게 힌트를 제공한다.
  - 사용자가 입력한 수와 컴퓨터가 생성한 수가 정확히 일치하면 재시작 여부를 묻는다.
  - 재시작 여부를 물을 때, 사용자가 1을 입력하면 게임을 재시작하고 2를 입력하면 종료시킨다.
