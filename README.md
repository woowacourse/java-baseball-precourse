...
1.  야구 게임에 사용할 수 있는 숫자 클래스.[ BaseBallNumber 클래스 ]
    - 범위는 1 - 9 사이.
    - 1 보다 작거나 9 보다 큰 수가 들어올 경우 exception 처리.
    - equals, hashcode 메서드를 override 하여 같은 baseballNumber끼리 비교 가능하게 함.
  

2.  사용자가 입력하는 세 자리 수 관리.  [ BaseBallNumbers 클래스 ]
    - 세자리 수가 아닐 경우 exception 처리.
    - 만들어진 세자리의 수는 컴퓨터 클래스에서 생성된 세자리의 수와 비교하여 볼 카운트와 스트라이크 카운트를 확인하는 메서드가 존재함.


3.  1에서 9까지 서로 다른 임의의 수 3개를 뽑아 세 자리 난수 생성. [ Computer 클래스 ]
    - 문제에서 제시한 '위 숫자 야구 게임에서 상대방의 역할을 컴퓨터가 한다.' 문구에 착안하여 만든 클래스
    - 세자리 난수 생성.
