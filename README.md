...
- [x] 사용자로부터 숫자를 입력받는다.
    ...
    - [x] [예외] 중복된 입력값이 있는 경우
- [x] 입력 값을 정답과 비교하여 결과(스트라이크, 볼, 낫싱)를 산출한다.
- [x] 결과를 출력한다.
    - 분기1: 결과가 3스트라이크 일 때
        - [x] 게임 종료 선언 메시지를 출력하고 게임 종료 Sequence를 진행한다.
    - 분기2: 결과가 3스트라이크가 아닐 때
        - [x] 게임 중 state를 반복한다.
        
State 3. 게임 종료
- [x] 게임 재시작 여부를 묻고 사용자로부터 입력을 받는다.
    - [x] [예외] 입력값이 1이나 2가 아닌 경우
    - 분기1: 입력 값이 1일 때
        - [x] 게임 시작 전으로 돌아가 순차적으로 진행한다.
    - 분기2: 입력 값이 2일 때
        - [x] 종료 메시지를 출력하고 프로그램을 종료한다.
