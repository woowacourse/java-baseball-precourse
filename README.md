...
# 🧑‍💻 기능 요구 사항
## 입출력
- [✅] 사용자에게 3개의 숫자 입력를 요구한다.
- [✅] 사용자로부터 3개의 숫자 입력을 받는다.
  - [✅] 이외의 입력은 예외(`IllegalArgumentException`)를 발생시킨다.
    - [✅] 3개의 숫자가 아닌 경우 (e.g. 0개, 2개, 4개)
    - [✅] 숫자가 아닌 값이 입력되는 경우 
    - [✅] 중복되는 숫자가 입력되는 경우
- [✅] 사용자에게 예측 결과를 안내한다. 
  - [✅] 게임의 룰에 따라 `X볼 X스트라이크` 혹은 `낫싱` 을 표시한다. 
  - [✅] 모두 맞출 경우 게임을 종료한다.
- [✅] 게임이 종료된 경우, 사용자에게 재시작 의도를 묻는다.
  - [✅] `1` 입력 : 게임을 새로 시작한다.
  - [✅] `2` 입력 : 게임을 종료한다.
  - [✅] 이외의 입력은 예외(`IllegalArgumentException`)를 발생시킨다.
    - [✅] 단일 문자 1, 2 이외의 값이 입력되는 경우

## 게임
- 컴퓨터
  - [✅] 게임 시작시 서로 다른 임의의 숫자 3개를 생성한다.
- [✅] 힌트
  - [✅] 동일한 자리에 동일한 숫자가 존재 : `스트라이크`로 처리
  - [✅] 다른 자리에 동일한 숫자가 존재 : `볼`로 처리
  - [✅] 다른 자리에 다른 숫자가 존재 : 처리하지 않음.
  - [✅] 자리에 관계없이, 동일한 숫자가 존재하지 않음 : `낫싱`으로 처리
...
- [✅] 3항 연산자를 쓰지 않는다.
- [⚠️] 함수(또는 메소드)가 한 가지 일만 하도록 최대한 작게 만들어라.
