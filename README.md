...
- `playGame()`: 먼저 랜덤한 세 숫자를 세팅한 뒤 `playBall()`를 실행하는 메소드이다. 이 후 1이 입력되면 게임을 새로 시작하고, 2가 입력되면 게임을 종료한다.
- `playBall()`: 숫자를 입력받고 입력값이 올바르지 않으면 에러를 처리한다. `countBall()` 메소드를 실행시킨 뒤, `ball`과 `strike` 값을 받아서 조건에 맞게 출력한다.
- `countBall()`: 랜덤하게 세팅된 세 숫자와 입력된 숫자로 볼과 스트라이크를 계산하는 메소드이다. 
