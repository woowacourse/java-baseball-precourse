...
### 게임 세팅
- [x] 컴퓨터는 1~9 범위의 랜덤 숫자를 생성한다.
  - 서로 다른 임의의 수 3개를 생성한다.
  - 만약 중복된 숫자가 있으면 다시 생성한다.
- [x] 컴퓨터의 랜덤 숫자와 사용자의 입력이 같으면 게임을 종료한다.
- [x] 첫 게임이 종료된 후, 재시작/종료 여부에 따라 게임을 반복한다.

### 입력
- [x] 사용자에게 1~9 범위의 숫자를 입력받는다.
- [x] 게임이 끝난 경우 재시작/종료 여부를 숫자 1 or 2로 입력받는다.
- [x] 사용자의 입력이 올바른지 검사한다.
  - 잘못된 값을 입력할 경우 `IllegalArgumentException`을 발생시킨다.
  - 애플리케이션을 종료한다.

### 출력
- [x] 사용자 입력에 대한 결과를 출력한다.
  - 볼 개수, 스트라이크 개수, 낫싱
- [x] 숫자를 모두 맞히면 성공 결과를 출력한다.
  - 게임을 종료하고, 반복 여부를 따른다.
...
#### 게임 플레이 중 사용자의 입력
- 3자리의 문자열이 아닌 경우
- 3자리의 문자열 중 1자리라도 1~9 범위를 벗어나는 경우
- 3자리의 문자열 중 중복된 숫자가 있는 경우

#### 게임 종료 후 사용자의 입력
- 1자리의 문자열이 아닌 경우
- 1자리의 문자열이 1~2 범위를 벗어나는 경우
