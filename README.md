...
## [🗨️ 1주차 회고](https://runa-nam.tistory.com/95)

## 📃 구현할 기능 목록

게임의 흐름은 '랜덤값 생성 > 입력 > 결과 확인 > (모두 맞췄을 시) > 종료여부 확인' 의 순서로 이루어진다.

1. 랜덤값 생성
    - 3자리 수를 랜덤으로 생성한다.
    - 각 자리는 1~9로 이루어진다.
    - 각 자리는 서로 다른 임의의 수로 구성되어있다.
    - 위의 조건들에 부합하지 않을 경우, 다시 랜덤값을 생성하도록 한다.

2. 입력 (야구게임의 랜덤값 추측)
    - 사용자에게 3자리 수를 입력 받는다.
    - 각 자리는 1~9의 수로 이루어진다.
    - 각 자리는 각각 다른 수로 입력받아야 한다.
    - [예외처리] 위의 조건들에 부합하지 않을 경우, `IllegalArgumentException`을 발생시킨 후 애플리케이션은 종료된다.

3. 결과 확인
    - 입력한 수와 랜덤값을 비교한 결과를 볼, 스트라이크 개수로 표시한다.
    - 입력한 수와 랜덤값에서 동일한 값이 없을 경우 낫싱을 출력한다.
    - 3개의 숫자를 모두 맞췄을 경우 게임을 새로 시작할지, 종료할지 여부를 묻는다.

4. 종료여부 확인
    - 다시 시작 시 1, 종료시 2를 입력받는다.
        - 다시 시작을 선택하였을 경우, 다시 입력으로 돌아간다.
    - [예외처리] 만약 입력받은 값이 1과 2가 아니라면 `IllegalArgumentException`을 발생시킨 후 애플리케이션은 종료된다.
