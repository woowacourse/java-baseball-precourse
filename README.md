...
- 입력
  - 3자리 수를 받아야 한다.
  - 게임이 끝난 경우에는 1 또는 2를 받아야 한다.
  - 예외 처리
    - 3자리 숫자가 아닌, 잘못된 값이 들어온 경우 `IllegalArgumentException`을 발생시킨 후 종료한다.
- 받은 입력에 대한 처리
  - 받은 입력값과 임의의 수를 비교하여 '볼', '스트라이크', '낫싱'을 확인한다.
  - 게임이 종료된 후에는 1 또는 2를 받아 다시 시작하거나 완전히 종료한다.
- 출력
  - 입력한 수에 대한 결과를 'n볼 n스트라이크'로 표시한다.
  - 하나도 맞추지 못한 경우 '낫싱'
  - 모두 맞춘 경우 '3스트라이크'
  - 게임이 종료된 경우 게임을 다시 시작할지 종료할지를 묻는다.
