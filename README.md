...
- [x] 1에서 9까지 서로 다른 수로 이루어진 3자리의 임의의 수를 생성하는 기능 구현
- [x] 입력한 수에 대한 결과를 출력
  - [x] 숫자, 자리가 모두 같은 수가 몇개인지 계산하는 기능 구현 => 스트라이크
  - [x] 숫자는 같으나 다른 자리에 있는 수가 몇개인지 계산하는 기능 구현 => 볼
  - [x] 같은 수가 전혀 없을 경우 ‘낫싱’ 출력
  - [x] 숫자를 모두 맞출 경우 메시지를 출력한 뒤 게임을 종료
- [] 잘못된 값을 입력받을 경우 `IllegalArgumentException` 예외를 발생하는 기능
  - [] 게임 진행 중 사용자로부터 입력받은 수가 3자리가 아닐 경우
  - [] 게임을 종료 후 계속할지 물었을 때 1이나 2가 아닌 다른 문자를 입력받았을 경우
