...
1. 출력
    - [ ] 숫자를 입력 받는 문구
    - [ ] 입력 받은 값을 정답과 비교하여 알려주는 문구
    - [ ] 정답일 때 알려주는 문구
    - [ ] 게임이 종료된 후 재시작을 알리는 문구
    
2. 입력
    - [ ] 세 자리 수를 입력 받기
        - [ ] 숫자가 아닌 경우 예외 처리
        - [ ] 중복되는 숫자가 있을 경우 예외 처리
        - [ ] 0이 포함되면 예외 처리
    - [ ] 게임 종료 후 취할 행동 선택지를 입력 받기
        - [ ] 선택지 외의 값을 입력 받았을 때 예외 처리
        
3. 값 비교
    - [ ] 입력 값과 정답의 값을 비교
        - [ ] Strike, Ball, Nothing 의 상태 반환
    - [ ] 정답과 일치할 경우
        - [ ] 정답 알림을 출력 후 게임 재시작 의사를 묻기