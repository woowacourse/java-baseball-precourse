...
- [X] 컴퓨터는 게임 시작 시 1에서 9까지 서로 다른 임의의 수 3개를 선택한다.
- [X] 게임 플레이어는 컴퓨터가 생각하고 있는 3개의 숫자를 입력한다.
    - [X] 예외 : 사용자가 잘못된 값을 입력하면 `IllegalArgumentException`을 발생시킨 후 애플리케이션을 종료한다.
    - [X] 예외 : 사용자가 중복된 값을 입력하면 `IllegalArgumentException`을 발생시킨 후 애플리케이션을 종료한다.
- [X] 게임 플레이어가 입력한 숫자에 대한 결과를 출력한다.
- [X] 게임 결과는 게임 플레이어가 입력한 3개의 숫자와 컴퓨터가 선택한 임의의 수 3개가 같은 수가 같은 자리에 있으면 스트라이크, 다른 자리에 있으면 볼, 같은 수가 전혀 없으면 낫싱이다.
- [X] 게임 플레이어가 컴퓨터가 선택한 3개의 숫자를 모두 맞힐때 까지 컴퓨터가 생각하는 3개의 숫자를 입력하고 결과를 출력하는 과정을 반복한다.
- [X] 게임 플레이어가 컴퓨터가 선택한 3개의 숫자를 모두 맞히면 게임이 종료된다.
    - [X] 모두 맞히면 "3개의 숫자를 모두 맞히셨습니다! 게임 종료" 멘트를 출력한다.
- [X] 게임을 종료한 후 게임을 다시 시작하거나(입력 : 1) 완전히 종료할 수 있다(입력 : 2).
    - [X] 예외 : 사용자가 잘못된 값을 입력하면 `IllegalArgumentException`을 발생시킨 후 애플리케이션을 종료한다.
