...
2. 3항 연산자 사용하지 않았는가?
3. 함수는 한 가지의 일만 하는가?
4. java convention에 어울리는가?
5. camp.nextstep.edu.missionutils에서 제공하는 Randoms, Console API를 활용하는가?
6. getter와 setter를 사용하지 않는가?
7. else를 사용하지 않는가?
8. 메서드의 인자가 3개 이하인가?
9. 모든 클래스의 인스턴스 변수가 3개 이하인가?
...
1. 같은 수가 같은 자리 -> 스트라이크
2. 같은 수가 다른 자리 -> 볼
3. 같은 수가 전혀 없다면 -> 포볼 or 낫싱
4. 컴퓨터가 1-9 임의의 수 3개를 선택
5. 플레이어가 컴퓨터의 숫자를 맞추는 형식
6. 플레이어가 입력하면 컴퓨터가 결과를 출력
7. 3개의 숫자를 모두 맞혀야 게임이 종료
8. 종료 후, 재시작 or 완전한 종료
9. 잘못된 입력 값 -> IllegalArgumentException 발생
10. 입출력 요구사항에 따라서 구문 작성
