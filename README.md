...
- #### 예외 처리
    ...
    - 사용자 추측 입력 문자열이 숫자 외 문자이거나, 유효 숫자 범위(1~9)를 벗어난 경우
    - 사용자 추측 입력 문자열의 3개의 수가, 모두 다르지 않은 경우
    - 게임 재진행 여부 구분자 입력이 유효 숫자 범위(1~2)를 벗어난 경우
    
- #### 야구 게임 룰 구현
    - 야구 게임의 룰에 따라, 사용자 추측 수의 점수를 표시한다.
    - 같은 수, 같은 자리 : 스트라이크
    - 같은 수, 다른 자리 : 볼
    - 같은 수가 없음 : 낫싱
    - 모두 같은 수인 경우 : 승리
