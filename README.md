...
- Random한 3자리 숫자 생성:
  ...
  `조건1` 1~9까지 각 자릿수가 서로 다른 3자리 수
  
  `조건2`  `Randoms.pickNumberInRange()` 를 이용한다.
  
  `예외처리` 한 번 사용한 숫자는 사용하지 않는다. 
  
- `출력`  "숫자를 입력하세요 : " 

- `Console.readLine()` 을 이용해 입력을 받는다.
  ...
  ​		`처리` : 각 항목에 해당하는 `IllegalArgumentException`을 발생
  
- 입력된 서로 다른 세 숫자에 대한 스트라이크 / 볼 판단:
  
  `조건1`  볼 = 해당하는 숫자가 존재하고 위치가 다를 경우
  
  `조건2`  스트라이크 = 해당하는 숫자가 존재하고 위치가 같을 경우
...
- 재실행 여부를 묻는다.
  ...
  `조건` 1을 입력하면 게임을 다시 시작하고, 2를 입력하면 종료한다.
  
  `예외` 1과 2 이외의 값을 입력하면 `IllegalArgumentException` 을 발생
