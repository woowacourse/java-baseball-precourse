...
- 우선, **Game, Computer, User** 클래스를 만들어준다.
- **[Main]**
    - Game 객체를 생성하여 playGame()의 반환값을 이용해 종료될 때 까지 게임을 계속한다.

- **[Computer]**
    - Random값을 추출하여 answer배열에 저장하여 답을 가지고 있는다.

- **[User]**
    - 입력값 예외 처리
        - 우선, 입력값의 길이가 정답의 길이와 다를 경우 IllegalArgumentException을 발생시킨다.
        - 입력값이 숫자가 아닐 경우 IllegalArgumentException을 발생시킨다.
        - 입력받은 숫자들 중 중복이 있다면 IllegalArgumentException을 발생시킨다.

- **[Game]**
    - Computer, User 객체를 한 개씩 생성하여 게임을 진행한다.
    - Computer가 가진 정답과 User가 적은 정답을 비교하고 결과를 출력한다(볼,스트라이크).
    - 정답이 맞을 때까지, User가 다시 정답을 적게한다.
    - 게임이 종료되면 게임을 계속할 것인지 묻고 boolean형으로 main으로 넘긴다.
