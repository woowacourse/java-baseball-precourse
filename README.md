...
* [x] 사용자 입력값 3자리수를 입력받는다.
  + [x] 입력값의 길이는 3이 아닌경우를 검증한다.
  + [x] 입력값이 서로 다른 수인지 검증한다.
  + [x] 입력값이 1~9 범위의 수인지 검증한다.
  + [x] 입력값이 숫자인지 검증한다.
* [x] 게임을 시작한다.
* [x] 컴퓨터 수와 사용자 수의 스트라이크 개수를 계산한다.
* [x] 컴퓨터 수와 사용자 수의 볼 개수를 계산한다.
* [x] 힌트(스트라이크, 볼, 낫싱)를 출력한다.
* [x] 정답 일때, 게임 재시작: 1 혹은 종료: 2 여부를 입력받는다.
  + [x] 정답 일때, 입력값을 검증한다.
