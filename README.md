...
- 1~9 숫자 3개를 입력한다.
  ...
  - 예외 : 숫자가 세 자리가 아닌 경우
  - 중복되는 숫자가 있는 경우는 예외로 처리하지 않는다.  
- 게임이 끝난 경우 재시작/종료 여부를 입력한다. (재시작: 1, 종료: 2)
  - 예외 : 1, 2 이외의 문자를 입력한 경우
- 입력 처리 : `camp.nextstep.edu.missionutils.Console`의 `readLine()`을 활용한다.
- 예외 처리 : `IllegalArgumentException`을 발생시킨 후 애플리케이션을 종료한다.

### 💻 컴퓨터 (상대방)
- 난수를 생성한다.
  - 1~9 숫자 중 **서로 다른** 3개의 숫자를 선택한다. 
  - 난수 처리 : `camp.nextstep.edu.missionutils.Randoms`의 `pickNumberInRange()`를 활용한다.
- 사용자가 입력한 숫자와 난수를 비교하여 결과를 출력한다.
  - 같은 숫자, 같은 자리일 경우 : `스트라이크`
  - 같은 숫자, 다른 자리일 경우 : `볼`
  - 모두 다른 숫자일 경우 : `낫싱`
  - 모두 같는 숫자, 같은 자리일 경우 : `3스트라이크` + `게임 종료`
- 게임이 종료될 경우 사용자의 입력 값에 따라 재시작 및 종료를 실행한다. 
