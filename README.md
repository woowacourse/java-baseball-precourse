...
- *Answer*
  - 1-9' 사이의 랜덤 수를 타깃 넘버로 생성한다
- *Player*
  - '1-9' 사이의 3자리 수를 입력받는다
  - 입력 값이 3자리 정수값인지 확인한다
  - 각 자리 수가 '1-9' 사이의 값이 맞는지 확인한다
- *Hint*
  - 스트라이크의 갯수를 카운트한다
  - 볼의 갯수를 카운트한다
  - 카운트한 결과를 토대로 힌트 문자열을 만든다
  - 힌트를 출력한다
- *Game*
  - Answer, Player, Hint 객체를 가지고 실제 게임을 실행한다
  - flag로 정답 여부를 확인하여 해당 게임을 종료한다
- *Application*
  - 게임의 시작과 종료를 컨트롤한다
- *Constant*
  - 기능 구현에 필요한 상수를 가지고 있는 클래스
