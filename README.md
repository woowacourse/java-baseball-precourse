...
### 사용자 입력 
  - 사용자 숫자 입력
    ...
    - 예외 사항
      - 자리 수가 맞지 않을 경우
        - 예) "1234"
      - 0 또는 숫자가 아닌 입력이 포함될 경우
        - 예) "012", "abc"
      - 같은 숫자가 두 개 이상 중복된 경우
        - 예) "111"
    
### 랜덤 숫자와 사용자 입력 비교 및 힌트 출력
  - 같은 숫자가 하나라도 존재
    - "n스트라이크" - n개의 숫자의 자리가 같은 경우
    - "m볼" - m개의 숫자의 자리가 다를 경우
    - "m볼 n스트라이크" - m개의 숫자의 자리가 다르고, n개의 숫자가 자리가 같은 경우
  - 같은 숫자가 존재하지 않음
    - "낫싱"
...
### 예외처리
  - 입력 조건을 어길 경우 `IllegalArgumentException`을 발생시킨 후 애플리케이션 종료
