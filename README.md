...
1. 컴퓨터의 3자리 수 결정
    - RandomUtils 클래스 활용해, 1~9로 구성된 임의의 수
    - 중복되지 않는 각 자릿수 List에 저장
2. 나의 3자리 수 입력 받고 저장
    - 예외처리 (IllegalArgumentException) : 비정상 입력 (1~9가 아닌 문자를 포함하거나 3자리가 아닌 경우, 0도 예외 발생)
    - 예외 경우 재입력 문구 출력과 함께 재입력 진행
    - 각 자릿수 List에 저장
3. 카운트한 볼과 스트라이크 값을 계산하고 저장한 List 반환
4. 결과 출력 메세지 결정
    - 0볼 0스트라이크의 경우 "낫싱"
5. 정답 여부 확인
    - 정답 여부는 boolean 변수에 저장
    - 정답 : 종료 메세지 출력
    - 오답 : 나의 3자리 수 입력으로 반복
6. 재시작 여부 확인
    - 재시작 여부 확인 문구 출력
    - 사용자 입력 값 확인
    - 예외처리 (IllegalArgumentException) : 비정상 입력 (1, 2외의 입력)
    - 예외 경우 재입력 문구 출력과 함께 재입력 진행
