...
자 이거 example에서 바꿨고 이제 원격저장소에 보낼거다! 먼저 저장해서 로컬저장소에 readme가 바꼈는지 확인해라
이게되면 앞으로 여기 example에서 수정하고(수정 후 저장하면 자동으로 로컬저장소 변경될거고) 로컬저장소에 있는걸 원격에 푸시하고

원격에 푸시된걸로 pull request를 보낸다 ]]


자이번에는 branch를 javajigi로 바꿔서 commit하여 push해보자~ 
