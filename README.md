...
- [ ] 유저 넘버와 컴퓨터의 저장된 값을 비교하여 볼의 개수를 판단하는 기능 구현
- [ ] 유저 넘버와 컴퓨터의 저장된 값을 비교하여 스트라이크의 개수를 판단하는 기능 구현
- [ ] 유저 넘버와 컴퓨터의 저장된 값을 비교하여 낫싱인지 판단하는 기능 구현
- [ ] 스트라이크 개수와 볼의 개수를 반환해준다. 만약 낫싱이면 낫싱을 반환한다.

### Game

- 게임이 진행되는 클래스이다.
- [ ] 3개의 숫자를 모두 맞히면 종료한 후, 입력으로 받은 값으로 게임을 재시작할지 종료할지 결정한다.

### Computer

- [ ] 생성된 랜덤 숫자를 저장하는 클래스이다.
...
- [ ] Referee로부터 받은 결과를 출력한다.
- [ ] 3개의 숫자를 모두 맞췄을때 게임 종료 멘트를 출력한다.

### 예외 처리

- 괄호안의 클래스 내에서 처리한다.
- 잘못된 입력이 들어올 경우 IllegalArgumentException 을 발생시키고 애플리케이션을 종료한다.
- [ ] (InputView) 유저의 입력값이 숫자가 아닌 문자를 입력받았을 때 예외 처리
- [ ] (InputView) 유저의 입력값이 3자리가 아닌 숫자가 들어왔을 때 예외 처리
- [ ] (InputView) 유저의 입력값에 중복이 있으면 예외처리
- [ ] (InputView) 유저의 입력값에 0 이 들어있으면 예외처리(1~9까지만 받는다.)
- [ ] (InputView) 게임 재시작 여부를 묻는 입력이 범위를 넘어갈때 예외 처리 (ex) 1, 2가 아닐떄)
