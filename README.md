...
### 📌 구현 기능 목록
```
1. 게임 시작 함수 startGame() 추가
2. 랜덤값 저장하는 기능 추가
3. 세자리 숫자를 입력받고 타입 검사 기능 추가
4. 랜덤값과 입력값이 같지 않을 때만 반복하는 while문 추가
5. 랜덤값과 입력값이 같지 않을 때 실행할 함수 compareValues() 추가 
6. 비교 함수 compareValues()에 기능 추가
   - 스트라이크 기능 추가 (같은 자리에 같은 숫자가 있다면)
   - 볼 기능 추가 (다른 자리에 같은 숫자가 있다면)
   - 낫싱 기능 추가 (같은 숫자가 하나도 없다면)
7. 게임 종료 후 재시작할지 질의하는 기능 추가
   - 1을 입력하면 재시작, 2를 입력하면 게임 종료
   - 재시작 시 startGame() 함수로 재귀
```
